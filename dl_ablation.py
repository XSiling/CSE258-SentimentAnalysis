...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.hidden_dim = 200
        self.n_layers = 5
        self.rnn = nn.RNN(input_size=300, hidden_size=200, num_layers=5, batch_first=True)
        self.linear = nn.Sequential(
            nn.Linear(200, 128),
            nn.Linear(128, 64),
            nn.Linear(64,2),
...
if __name__ == '__main__':
    ...
    train_data=mydataset(filename='train-ablation.pkl')
    valid_data=mydataset(filename='valid-ablation.pkl')
    test_data=mydataset(filename='test-ablation.pkl')
    ...
    for epoch in range(opt.epoch):
        ...
        if accuracy>BEST_VALID_ACC:
            ...
            torch.save(model.state_dict(), "best_" + opt.model_type + "ablation.model")
